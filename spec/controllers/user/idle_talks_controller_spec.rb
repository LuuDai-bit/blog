...
RSpec.describe User::IdleTalksController, type: :controller do
  describe 'GET #index' do
    ...
    context 'without search and filter' do
      let!(:posts) do
        create_list(:post, 5, status: :publish, release_date: Time.current, type: 'IdleTalk')
      end
      let!(:post) { create(:post, status: :draft, type: 'IdleTalk') }
    ...
    context 'when search by post subject' do
      ...
      let!(:post_search_subject) do
        create(:post, subject: 'Search subject', status: :publish, release_date: Time.current, type: 'IdleTalk')
      end
    ...
    context 'when filter by subject_en' do
      ...
      let!(:post_search_subject) do
        create(:post, subject_en: 'Search subject', status: :publish, release_date: Time.current, type: 'IdleTalk')
      end
      let(:params) do
        {
    ...
    context 'when filter by category' do
      ...
      let!(:posts) { create_list(:post, 5, status: :publish, release_date: Time.current, type: 'IdleTalk') }
      let(:post) { create(:post, status: :publish, release_date: Time.current, type: 'IdleTalk') }
